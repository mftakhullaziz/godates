...
func InitializeRouter(authHandler *authHandler.AuthHandler) *http.ServeMux {
	r := http.NewServeMux()
	...
	r.Handle("GET /godating-dealls/api/daily-accounts", md.AuthMiddleware(http.HandlerFunc(nil)))
